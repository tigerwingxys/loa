...
class Game {
	...
	/**
	 * Controller for one or more games of LOA, using MANUALPLAYERTEMPLATE as an
	 * exemplar for manual players (see the Player.create method) and
	 * AUTOPLAYERTEMPLATE as an exemplar for automated players. Reports board
	 * changes to VIEW at appropriate points. Uses REPORTER to report moves,
	 * wins, and errors to user. If LOGFILE is non-null, copies all commands to
	 * it. If STRICT, exits the program with non-zero code on receiving an
	 * erroneous move from a player.
	 */
	...
	private static final Pattern COMMAND_PATN = Pattern
	...
	/**
	 * Set square S to CONTENT ('black', 'white', or '-'), and next player to
	 * move to NEXTPLAYER: 'black' or 'white'.
	 */
	...
	/**
	 * Perform the move designated by LINE, if a valid move. Return true iff
	 * LINE has the syntax of a move.
	 */
	...
	/**
	 * Print an announcement of the winner. Requires that the game has been won.
	 */
	...
	/**
	 * A player to handle commands when game not started. Does not actually
	 * generate moves.
	 */
	private Player _nonplayer;

	/**
	 * A source of pseudo-random numbers, primed to deliver the same sequence in
	...
	/**
	 * If true, command errors cause termination with error exit code.
	 */
